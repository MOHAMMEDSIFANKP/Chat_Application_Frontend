...
function PeopleList() {
  ...
  const [profilefriends_list, setprofilefriends_list] = useState([])
  ...
  const GetProfileFunc = async (id) => {
    try {
      ...
      if (res2.status === 200) {
        ...
        setprofilefriends_list(res2.data?.data?.friends_list)
      }
    } catch (error) {
      console.log(error);
    }
  ...
  useEffect(() => {
    if (type_list) {
      setSelectedOption(type_list)
    } else {
      setSelectedOption('People')
    }
            ...
            <List>
              ...
              {users.length > 0 ? (
                users.map((user, index) => (
                  <>
                    <div key={index} className={`grid grid-cols-[1fr,5.5rem] my-1 cursor-pointer ${profile.email === user.email ? 'bg-gray-200 rounded-lg' : 'hover:bg-gray-200 hover:rounded-lg'} `}>
                      <div className='grid grid-cols-[4.5rem,1fr] py-3 ' onClick={() => GetProfileFunc(user.id)}>
                ...
                <Card className='mx-5 mt-5'>
                  <Accordion open={open === 1} icon={<Icon id={1} open={open} />}>
                    ...
                    <AccordionBody className='max-h-[16rem] overflow-auto'>
                      <List>
                        {profilefriends_list.length > 0 ? profilefriends_list.map((user, index) => (
                          <>
                            <div key={index} className='grid grid-cols-[1fr,5.5rem] my-1 hover:bg-gray-200 hover:rounded-lg cursor-pointer'>
                              <div className='grid grid-cols-[4.5rem,1fr] py-3 ' onClick={() => UserInfo.email === user.email ? '' : GetProfileFunc(user.id)}>
                                <div className='flex justify-center items-center'>
                                  <Avatar
                                    variant="circular"
                                    alt="candice"
